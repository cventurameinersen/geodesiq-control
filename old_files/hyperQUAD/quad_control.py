...
class HyperQuadControl:
    ...
    def __init__(self, hamiltonian: Optional[np.ndarray] = None, partial_hamiltonian: Optional[np.ndarray] = None,
                 ...
                 control_init: Optional[np.ndarray] = None, control_final: Optional[np.ndarray] = None,
                 pulse_accuracy: int = 2 ** 16 + 1, discrete_length: int = 4, alpha: float = 2.0, beta: float = 2.0,
                 dia_alpha: float = 0.0, dia_beta: float = 0.0, pulse_time: float = 10,
                 control_method: str = 'hyperQUAD', ham_args: Tuple[Any, ...] = (),
                 ham_kwargs: Optional[Dict[str, Any]] = None, diad_list: Optional[np.ndarray] = None,
                 custom_pulse: Optional[np.ndarray] = None, ODE_method: str = 'RK45', integration_method: str = 'romb',
    ...
    def update_params(self, **kwargs):
        ...
        kwargs : dict
            ...
            - 'qsn_variance'
        """
        valid_params = {'alpha', 'beta', 'dia_alpha', 'dia_beta', 'pulse_accuracy', 'discrete_length', 'control_init',
            'control_final', 'pulse_time', 'hamiltonian', 'partial_hamiltonian', 'ham_args', 'ham_kwargs',
            'initial_state', 'final_state', 'control_method', 'custom_pulse', 'ODE_method', 'integration_method',
    ...
    def _generate_hyperarrays(self):
        ...
        self._full_hamiltonian = np.array(
            [self.hamiltonian(control_val, *self.ham_args, **self.ham_kwargs) for control_val in self._control_vals])
        # self._full_partial_hamiltonian = np.array([
        #     self.partial_hamiltonian for _ in range(len(self._control_vals))
        # ])
        self._full_partial_hamiltonian = np.array(
            [self.partial_hamiltonian(control_val, *self.ham_args, **self.ham_kwargs) for control_val in
                self._control_vals])
    ...
    def plot_eigenvalues(self, shift=1, linewidth=2, bdry: bool = True):
        ...
        mod_control_vals = np.linspace(control_vals[0] + np.sign(control_vals[0]) * shift,
                                       control_vals[-1] + np.sign(control_vals[-1]) * shift, self.pulse_accuracy)
    ...
    def generate_adiabatic_params(self):
        ...
        def G_tensor(energies, states, num, dim):
            ...
            for n in range(dim):
                if n != m:
                    num = np.abs(np.einsum('ia,iab,ib->i', states[..., m].conj(), full_partial_ham, states[..., n],
                                           optimize='greedy'))
    ...
    def compute_optimal_pulse(self):
        ...
        factor_interpolation = interp1d(control_vals, a_tilde / np.sqrt(G_tensor), kind='quadratic',
        ...
        sol = solve_ivp(model, [0, 1], [control_vals[0]], t_eval=s, method=self.ODE_method, atol=1e-15, rtol=1e-13,
                        dense_output=True)  # atol=1e-8 (1e-15), rtol=1e-4 (1e-12)
        s = sol.t
        control_sol = sol.y[0]
    ...
    def plot_pulse(self):
        ...
        if self.control_method == 'hyperQUAD' or self.control_method == 'diad' or self.control_method == 'discrete_hyperQUAD' or self.control_method == 'filtered_hyperQUAD':
            ...
            if self.custom_pulse is None:
                raise ValueError(
                    "Custom pulse not set. Please set 'custom_pulse' before using 'custom' control method.")
            else:
                s = np.linspace(0, 1, self.pulse_accuracy)
                control_sol = self.custom_pulse

        plt.figure(figsize=(6, 4))
        plt.plot(s, control_sol, linewidth=2,
                 label=fr"$\alpha, \beta, \hat{{\alpha}}, \hat{{\beta}}$ = {self.alpha}, {self.beta}, {self.dia_alpha}, {self.dia_beta}")
    ...
    def plot_metric_tensor(self):
        ...
        plt.figure(figsize=(6, 4))
        plt.plot(control_vals, metric_tensor, linewidth=2,
                 label=fr"$\alpha, \beta, \hat{{\alpha}}, \hat{{\beta}}$ = {self.alpha}, {self.beta}, {self.dia_alpha}, {self.dia_beta}")
    ...
    def compute_fidelity(self, c_ops=None, infidelity: bool = False, quasistatic: bool = False,
                         qsn_parallel: bool = False):
        """
        ...
        def _compute_fidelity(control_sol, times, c_ops, infidelity, quasistatic):
            ...
            H_T = qt.QobjEvo(H_t_func, args={"ham_args": self.ham_args, "x_array": qsn_control_sol, "t_array": times})
        ...
        if quasistatic and (not qsn_parallel):
            ...
            fidelities = Parallel(n_jobs=int(num_samples / 4))(
                delayed(_compute_fidelity)(control_sol, times, c_ops, infidelity, quasistatic=True) for _ in
                range(num_samples))
        ...
        # Define fully time-dependent Hamiltonian  # def H_t_func(t, args):  #     ham_args = args["ham_args"]  #     x_array = args["x_array"]  #     t_array = args["t_array"]  #     x_t = np.interp(t, t_array, x_array)  #     return qt.Qobj(self.hamiltonian(x_t, *ham_args))

        # qsn_control_sol = control_sol*(1+np.random.normal(0, self.qsn_variance)) if quasistatic else control_sol

        # H_T = qt.QobjEvo(H_t_func, args={  #     "ham_args": self.ham_args,  #     "x_array": qsn_control_sol,  #     "t_array": times  # })

        # # Solve dynamics  # psi_f = qt.mesolve(H_T, psi_init, times, c_ops=c_ops).states[-1]

        # # Return fidelity or infidelity  # fid = qt.fidelity(psi_target, psi_f) ** 2  # return 1 - fid if infidelity else fid
    ...
    def fidelity_sweep(self, sweep_dict: Dict, c_ops=None, infidelity: bool = False, quasistatic: bool = False,
                       qsn_parallel: bool = False):
        """
        ...
        original_params = {'alpha': self.alpha, 'beta': self.beta, 'dia_alpha': self.dia_alpha,
            'dia_beta': self.dia_beta, 'pulse_accuracy': self.pulse_accuracy, 'control_init': self.control_init,
            'control_final': self.control_final, 'hamiltonian': self.hamiltonian,
            'partial_hamiltonian': self.partial_hamiltonian, 'ham_args': self.ham_args,
            'ham_kwargs': self.ham_kwargs.copy(), 'initial_state': self.initial_state, 'final_state': self.final_state,
        ...
        if len(sweep_keys) == 1:
            ...
            if quasistatic:
                for val in tqdm(values, desc=f"Sweeping {key}"):
                    ...
                    mean_fid, median_fid, std_fid = self.compute_fidelity(c_ops=c_ops, infidelity=infidelity,
                                                                          quasistatic=quasistatic,
                                                                          qsn_parallel=qsn_parallel)
                ...
                for val in tqdm(values, desc=f"Sweeping {key}"):
                    self._apply_sweep_value(key, val)
                    fidelity = self.compute_fidelity(c_ops=c_ops, infidelity=infidelity, quasistatic=quasistatic,
                                                     qsn_parallel=qsn_parallel)
                    fidelities.append(fidelity)
                self.update_params(**original_params)
        ...
        # 2D Sweep
            ...
            for i, val1 in outer_loop:
                ...
                for j, val2 in inner_loop:
                    ...
                    fidelities[i, j] = self.compute_fidelity(c_ops=c_ops, infidelity=infidelity,
                                                             quasistatic=quasistatic)
    ...
    def fidelity_sweep_parallel(self, sweep_dict: Dict, c_ops=None, infidelity: bool = False, n_jobs: int = 3):
        sweep_params = {k: v for k, v in sweep_dict.items() if hasattr(v, '__len__') and not isinstance(v, str)}
        fixed_params = {k: v for k, v in sweep_dict.items() if k not in sweep_params}
        sweep_keys = list(sweep_params.keys())

        original_params = {'alpha': self.alpha, 'beta': self.beta, 'pulse_accuracy': self.pulse_accuracy,
            'control_init': self.control_init, 'control_final': self.control_final, 'hamiltonian': self.hamiltonian,
            'partial_hamiltonian': self.partial_hamiltonian, 'ham_args': self.ham_args,
            'ham_kwargs': self.ham_kwargs.copy(), 'initial_state': self.initial_state, 'pulse_time': self.pulse_time, }
        ...
        # 2D Sweep
            ...
            results = Parallel(n_jobs=n_jobs)(delayed(compute_single_2d)(i, j, val1, val2) for i, j, val1, val2 in
                                              tqdm(tasks, desc="Parallel 2D sweep"))
